      ...
      <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden hover:border-blue-600 transition-all duration-300 active:scale-[0.98] group-hover:shadow-2xl group-hover:shadow-blue-50">
        ...
        <div className="p-6">
          ...
          <div className="flex items-start justify-between gap-4 mb-4">
            <h2 className="text-base font-black text-gray-900 leading-tight line-clamp-2 flex-1 group-hover:text-blue-700 transition-colors">
              {course.title}
            </h2>
            <span className="shrink-0 px-3 py-1 bg-sky-50 text-blue-400 text-[10px] font-black uppercase tracking-[0.2em] rounded-lg border border-sky-100 transition-colors group-hover:bg-blue-600 group-hover:text-white group-hover:border-blue-600">
              {course.code}
            </span>
          </div>
          ...
          <div className="flex items-center gap-4 pt-4 border-t border-sky-50">
            {course.days && (
              <div className="flex items-center gap-1.5 opacity-60 group-hover:opacity-100 transition-all">
                ...
                <span className="text-[10px] text-blue-900 font-black uppercase tracking-widest">{course.days}</span>
              </div>
            )}
            {course.time && (
              <div className="flex items-center gap-1.5 opacity-60 group-hover:opacity-100 transition-all">
                <Clock className="h-3.5 w-3.5 text-blue-600 shrink-0" />
                <span className="text-[10px] text-blue-900 font-black uppercase tracking-widest">{course.time}</span>
              </div>
            )}
